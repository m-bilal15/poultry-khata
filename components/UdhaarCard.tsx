          ...
          <div className="flex items-center gap-3">
            <div className="text-left">
              ...
              <p className="text-xs text-gray-400 text-right">
                {isOwed ? t('due') : balance < 0 ? t('overpaid') : t('clear')}
              </p>
            </div>
        ...
        <div className="border-t border-gray-50 px-4 pb-4 pt-3 space-y-4">
          {customer.phone && isOwed && (
            <div className="flex justify-end">
              <WhatsAppButton phone={customer.phone} message={waMessage} label={t('sendReminder')} size="sm" />
            </div>
          )}

          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-2xl p-3 space-y-2" style={{ background: '#fef2f2' }}>
              <p className="text-xs font-bold text-red-600 text-center">{t('creditGiven')}</p>
              ...
              <input value={debitForm.note}
                ...
                className="w-full bg-white border border-red-100 rounded-xl px-3 py-2 text-xs"
                placeholder={t('noteShort')} />
              <button
            ...
            <div className="rounded-2xl p-3 space-y-2" style={{ background: '#f0fdf4' }}>
              <p className="text-xs font-bold text-green-600 text-center">{t('paymentReceived')}</p>
              ...
              <input value={creditForm.note}
                ...
                className="w-full bg-white border border-green-100 rounded-xl px-3 py-2 text-xs"
                placeholder={t('noteShort')} />
              <button
          ...
          {entries.length > 0 && (
            <div>
              <p className="text-xs font-bold text-gray-400 mb-2">{t('txHistory')}</p>
